...
const HomeCategoryList = ({ categories, isLoading }: PropTypes) => {
  return (
    <Card className="mx-6 mb-8 lg:mx-0">
      <CardHeader className="p-0">
        <h1 className="text-danger text-2xl font-bold">Event By Category</h1>
      </CardHeader>
          ...
          {!isLoading
            ? categories?.map((category) => (
                <Link
                  ...
                  className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-amber-50 p-4 shadow-lg hover:shadow-lg"
                >
                  <Image
                    src={`${category.icon}`}
                    alt={`${category.name}`}
